...
- **Slopes still need local ground estimation.** `classify_obstacle` compares against a single global `ground_z` (the robot's `base_link` z), not local terrain. The rim test keeps modest downgrades from being called trenches, but on a sustained grade the whole classification drifts. Related and separate: `Z_REL_MIN = -2.0` is measured from `ground_z`, so on an 8% downgrade every return past ~27 m falls below it and is **discarded outright** — measured: 65 of 116 points on a 60 m probe. Local ground estimation fixes both.
