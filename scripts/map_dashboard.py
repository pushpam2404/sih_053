...
moving objects, and writes ONE self-contained HTML file (no external assets) with:
  ...
  * static vs dynamic object tracks
...
from drdo_lidar_mapping.perception.cluster import cluster_hard_obstacles  # noqa: E402
from drdo_lidar_mapping.perception.tracker import SortTracker  # noqa: E402
...
DYN_SPEED = 1.0                          # m/s: tracks faster than this are reported as dynamic
...
def layer_colours(cells, frame_ts):
    ...
    for arr in (sem, ele, cost):
        arr[stale, :3] = (arr[stale, :3] * 0.45).astype(np.uint8)          # decaying / unseen > 5 s
    return {"semantic": sem, "elevation": ele, "cost": cost}
...
def rasterise(cells, colours, cx, cy, half_m, px_m, min_outline_px):
    ...
    orders["semantic"] = orders["elevation"] = np.lexsort((finer, cells["last_update_ts"].astype(np.int64), hit))
    out = {}
...
def main():
    ...
    tracker = SortTracker(max_age=3, min_hits=3, iou_threshold=0.05, dt=1.0 / HZ)
    drdo_map.reset_map()
    ...
    for f in range(1, args.frames + 1):
        ...
        near = np.hypot(scan[:, 0] - sx, scan[:, 1] - sy) <= 60.0
        hard = scan[near & (scan[:, 3] == 4)]
        dets = cluster_hard_obstacles(hard[:, :3], np.full(len(hard), 4), eps=0.8, min_samples=5)
        det_arr = np.array([d.bbox_2d for d in dets], np.float32).reshape(-1, 4)
        tracks = tracker.update(det_arr)
        dyn = [tr for tr in tracks if np.hypot(tr[4], tr[5]) > DYN_SPEED]
        # Points inside confirmed dynamic tracks do not enter the static map (otherwise a walking person
        # leaves a lethal trail until temporal decay clears it).
        mask = np.ones(len(scan), bool)
        for tr in dyn:
            mask &= ~((np.abs(scan[:, 0] - tr[0]) <= tr[2] / 2 + 0.3) & (np.abs(scan[:, 1] - tr[1]) <= tr[3] / 2 + 0.3)
                      & (scan[:, 3] == 4))
        ...
        movers = [box_at(b, t) for b in boxes if b[8] in ("pedestrian", "vehicle")]
        for tr in dyn:
            dyn_track_frames += 1
            false_dyn_frames += not any(np.hypot(tr[0] - mx, tr[1] - my) < 3.0 for mx, my in movers)
        for b in boxes:
            if b[8] not in ("pedestrian", "vehicle"):
                continue
            bx, by = box_at(b, t)
            rng_m = float(np.hypot(bx - sx, by - sy))
            if rng_m > 60.0 or f < 10:
                continue
            found = any(np.hypot(tr[0] - bx, tr[1] - by) < 1.5 for tr in dyn)
        ...
        if f % snap_every == 0 or f == args.frames:
            ...
            truth = [{"name": b[8], "x": box_at(b, t)[0] - sx, "y": box_at(b, t)[1] - sy}
                     for b in boxes if b[8] in ("pedestrian", "vehicle")]
            snapshots.append({
                ...
                "tracks": [{"x": float(tr[0] - sx), "y": float(tr[1] - sy), "w": float(tr[2]), "h": float(tr[3]),
                            "speed": float(np.hypot(tr[4], tr[5])), "id": int(tr[6]),
                            "dynamic": bool(np.hypot(tr[4], tr[5]) > DYN_SPEED)} for tr in tracks],
                "truth": truth,
            })
            print(f"[DASH] frame {f:4d}  cells={len(cells['ix']):7d}  levels={lv_counts}  "
                  f"load={drdo_map.load_factor():.3f}  tracks={len(tracks)} dynamic={len(dyn)}")
    ...
    with open(args.out, "w") as fh:
        fh.write(HTML.replace("__DATA__", json.dumps({"summary": summary, "snapshots": snapshots})))
    ...
    recall_txt = ", ".join(f"{r['range']}=" + ("n/a" if r["recall"] is None else f"{100 * r['recall']:.0f}%")
                           for r in by_range)
    print(f"[DASH] dynamic-object recall by range: {recall_txt}; false dynamic tracks on static objects: "
          f"{false_dyn_frames}/{dyn_track_frames} track-frames")
...
<div class="controls">
 ...
 <button data-layer="semantic" class="on">Semantic</button><button data-layer="elevation">Elevation</button><button data-layer="cost">Cost</button>
</div>
<div class="grid">
 ...
 <div class="panel"><h2>Far field ±100 m — tracks: <span style="color:#ff5ad2">dynamic</span> / <span style="color:#fff">static</span></h2><div class="view"><img id="far"><canvas id="farc" width="800" height="800"></canvas></div>
 ...
 <div class="panel"><h2>Dynamic-object detection by range</h2><table id="dyn"></table><div class="note">DBSCAN on OBSTACLE_HARD points + SORT tracker; dynamic = track speed &gt; 1 m/s, matched within 1.5 m of the true object. Points of dynamic tracks are kept out of the static map.</div></div>
 <div class="panel"><h2>Cells per resolution level (current frame)</h2><table id="lv"></table></div>
</div>
<div class="note" style="margin-top:12px">Synthetic scene, OS1-64 beam pattern, no occlusion. Point labels are simulator ground truth — the segmentation network is not in this loop (see scripts/eval.py). Generated by scripts/map_dashboard.py.</div>
<script>
const D=__DATA__, S=D.summary, snaps=D.snapshots; let idx=snaps.length-1, layer='semantic', timer=null;
...
document.getElementById('dyn').innerHTML='<tr><th>range</th><th>object-frames</th><th>recall</th></tr>'+S.dynamic_recall_by_range.map(r=>`<tr><td>${r.range}</td><td>${r.frames}</td><td>${r.recall==null?'n/a':fmt(100*r.recall,0)+'%'}</td></tr>`).join('')+`<tr><td>static objects reported dynamic</td><td>${S.false_dynamic_track_frames} / ${S.dynamic_track_frames}</td><td>track-frames</td></tr>`;
document.getElementById('legend').innerHTML=S.classes.map(c=>`<span><i class="sw" style="background:rgb(${c.rgb})"></i>${c.name}</span>`).join('')+'<span><i class="sw" style="background:#2d343e"></i>FREE (carved)</span>';
...
function overlay(cv,half,s,rings){const g=cv.getContext('2d'),W=cv.width,k=W/(2*half);g.clearRect(0,0,W,W);g.lineWidth=Math.max(1,W/600);
 ...
 s.tracks.forEach(t=>{if(Math.abs(t.x)>half||Math.abs(t.y)>half)return;g.strokeStyle=t.dynamic?'#ff5ad2':'rgba(255,255,255,.8)';g.lineWidth=Math.max(2,W/300);
  const w=Math.max(t.w*k,6),h=Math.max(t.h*k,6);g.strokeRect(W/2+t.x*k-w/2,W/2-t.y*k-h/2,w,h);
  if(t.dynamic){g.fillStyle='#ff5ad2';g.font=`${Math.max(12,W/60)}px system-ui`;g.fillText(`#${t.id} ${t.speed.toFixed(1)} m/s`,W/2+t.x*k+w/2+3,W/2-t.y*k)}});
